...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            for file_path in files:
                try:
                    self.index_single_file(str(file_path), clear_cache=False)
            ...
            if result.success_count:
                self._clear_grounding_cache(reason=f"reindexed_directory:{dir_path.name}")
    ...
    def index_knowledge_base(self) -> IndexingResult:
        """
        索引项目内置知识库目录。

        默认只索引 docs/knowledge_base 目录下的一层 .md/.txt 文件，不递归索引 source
        母文件目录，避免母文档和切片文档重复入库。
        """
        return self.index_directory(self.knowledge_base_path)

    def index_single_file(self, file_path: str, clear_cache: bool = True):
        """
        ...
        try:
            ...
            if clear_cache:
                self._clear_grounding_cache(reason=f"reindexed_file:{path.name}")

        except Exception as e:
            logger.error(f"索引文件失败: {file_path}, 错误: {e}")
            raise RuntimeError(f"索引文件失败: {e}") from e

    def _clear_grounding_cache(self, reason: str) -> None:
        try:
            from app.services.rag_agent_service import rag_agent_service

            rag_agent_service.clear_grounding_cache(reason=reason)
        except Exception as exc:
            logger.warning(f"清理知识库预检索缓存失败: reason={reason}, error={exc}")
