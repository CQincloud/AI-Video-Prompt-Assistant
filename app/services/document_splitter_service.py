...
class DocumentSplitterService:
    ...
    def __init__(self):
        ...
        self.markdown_section_max_size = 7600
    ...
    def split_markdown(self, content: str, file_path: str = "") -> List[Document]:
        ...
        try:
            ...
            # 第二阶段: 只切分超大的标题段，保留完整语义清单
            docs_after_split = self._split_large_markdown_sections(md_docs)
    ...
    def _split_large_markdown_sections(self, documents: List[Document]) -> List[Document]:
        """Keep Markdown heading sections intact unless they exceed the storage limit."""
        docs_after_split: List[Document] = []
        for doc in documents:
            if len(doc.page_content) <= self.markdown_section_max_size:
                docs_after_split.append(doc)
                continue
            docs_after_split.extend(self.text_splitter.split_documents([doc]))
    ...
    def _merge_small_chunks(
        ...
        for doc in documents:
            ...
            if current_doc is None:
                ...
                and not self._starts_with_markdown_heading(doc)
            ):
                # 当前文档太小且合并后不会太大，则合并
    ...
    def _starts_with_markdown_heading(self, document: Document) -> bool:
        first_line = document.page_content.lstrip().splitlines()[0]
