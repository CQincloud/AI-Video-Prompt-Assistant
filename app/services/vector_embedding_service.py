...
class DashScopeEmbeddings(Embeddings):
    ...
    def __init__(
        ...
        logger.info(
            ...
            f"模型: {model}, 维度: {dimensions}, API Key: configured"
        )
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            for start in range(0, len(texts), self.MAX_BATCH_SIZE):
                ...
                logger.debug(
                    f"嵌入批次: {start + 1}-{start + len(batch)} / {len(texts)}"
                )
            ...
            logger.error(f"批量嵌入失败: {type(e).__name__}")
            raise RuntimeError("批量嵌入失败") from e
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"查询嵌入失败: {type(e).__name__}")
            raise RuntimeError("查询嵌入失败") from e
