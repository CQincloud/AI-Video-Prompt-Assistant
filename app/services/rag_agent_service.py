...
class RagAgentService:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return dedent("""
            ...
            当任务是“场景提示词 / 场景图 / 空场景资产图”时，改用导演级场景结构：
            【理解你的需求】
            【知识库依据】
            【导演级场景拆解】
            【示例提示词：导演级空场景资产图】
            正向提示词 / 负面提示词
            【回答自检】
            场景资产图任务不得输出【视频创作拆解】、【可选增强】、动态化建议、声音设计或视频镜头调度，除非用户明确要求视频分镜。
        """).strip()
    ...
    def _build_grounded_user_prompt_with_meta(
        ...
        *,
        ...
        extra_contracts: list[str] = [
            "- 可复制提示词区块必须自包含，并含“正向提示词”和“负面提示词”；同一次回答只允许一套最终可复制提示词，不得在后面追加第二套压缩版或二次整理版。\n"
        ]
        if is_character_prompt:
            skin_rule = (
        ...
        if is_scene_prompt:
            extra_contracts.append(
                ...
                "最终标题必须使用【示例提示词：导演级空场景资产图】，不得使用旧标题【示例提示词：场景画面设定】。"
                "最终提示词必须按【美学】【主题】【时间天气】【尺度】【前景】【中景】【主体预留区】【远景】【关键元素】【光影】【色彩】【镜头质感】【避免】【比例】输出。"
                ...
                "人物、职业、动作和对话只能用于推断地点功能、环境状态、主体预留区和痕迹；例如门半开、卷宗散落、雨水脚印、灯火未熄、桌前留出空椅，不得让人物入画。"
                ...
                "必须锁定原文事实：时间、天气、地点、建筑材质、窗户类型和道具状态只能来自用户或剧本；不得把黄昏改成夜晚/月光，不得把晴天写成雨天，不得把未写雨的场景改成雨夜，不得把木窗改成玻璃窗。"
                "必须判断道具生命周期：请柬、银钗、旧画、星图、卷宗等道具是隐藏、静置、递出、展开还是收回；空场景图只能写当前空间中合理存在或预留的位置，不得写“手中的银钗”等人物持有状态。"
                "负面词必须排除人物、角色、行人、人群、背影、人体、脸、手、脚、剪影、演员、模特、站立人物、走动人物。"
                "文字、水印、标识类限制按画面用途条件加入；牌匾、卷宗、报纸、招牌、门牌等文字载体可作为道具存在，但不要强制生成可读文字。"
                "【比例】字段必须同时写场景图比例和视频比例；所有场景统一写横版 16:9，不再输出其他比例；核心视觉锚点、关键道具和主体预留区处于横屏安全构图内，并保留中轴可裁切空间。"
                "色调可作为文字方案；除非用户要求，不让画面出现色卡块。"
                "场景资产图任务不得输出【视频创作拆解】、【可选增强】、动态化建议、声音设计或视频镜头调度；这些只在用户明确要求视频分镜时输出。\n"
            )
