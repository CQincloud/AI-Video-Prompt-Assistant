...
class MilvusSchemaMismatchError(RuntimeError):
    """Raised when an existing Milvus collection is incompatible with config."""
...
class MilvusClientManager:
    ...
    def _handle_vector_dimension_mismatch(self, existing_dim: int) -> None:
        message = (
            f"Milvus collection '{self.COLLECTION_NAME}' vector dimension mismatch: "
            f"existing={existing_dim}, configured={self.VECTOR_DIM}."
        )
        if not config.milvus_allow_destructive_schema_reset:
            raise MilvusSchemaMismatchError(
                f"{message} Refusing to drop the existing collection. "
                "Run a reviewed migration/reindex job, use a new collection name, or set "
                "MILVUS_ALLOW_DESTRUCTIVE_SCHEMA_RESET=true only in disposable local development."
            )

        logger.warning(
            f"{message} MILVUS_ALLOW_DESTRUCTIVE_SCHEMA_RESET=true; "
    ...
    def _drop_and_recreate_collection(self) -> None:
        if self._collection is not None:
            try:
                ...
                logger.warning(f"Failed to release collection before schema reset: {exc}")
