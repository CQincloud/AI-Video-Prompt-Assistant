class SuperBizAgentApp {
    constructor(currentUser = null) {
        ...
        this.loadPromptTemplatesFromServer();
        this.updateUI();
...
请按后端角色生成规则输出。保留用户指定画风；未指定画风时默认真人写实风格。`,
                instruction: `请你作为 AI 视频角色提示词导演，根据用户输入生成一个适合 AI 真人视频/写实视频生产的完整人物设定与提示词。

你的目标不是只写“漂亮、冷酷、温柔、神秘”这类抽象标签，而是让角色具有“活人感”：有身份、有状态、有微表情、有潜动作、有环境互动、有真实光影和物理细节。

画风规则：
- 如果用户没有明确写入画风、影像风格或美术风格，最终示例提示词必须默认为“真人写实风格”，并在正向提示词里明确写入真人写实、电影感、真实光影、真实材质。
- 如果用户已经明确写入画风，例如二次元、国漫、水墨、赛博朋克、皮克斯、迪士尼、Webtoon、胶片、厚涂等，必须按用户指定画风生成，不要再强行改成真人写实。

你需要重点补充：
1. 身份与背景：角色来自哪里，处于什么关系和处境；
2. 外貌识别点：年龄感、脸型、五官、发型、身形、肤色、独特标记；
3. 服装与道具：服装材质、层次、颜色、配饰、随身物；
4. 性格气质：表面气质 + 隐藏性格；
5. 微表情：眼神变化、眉毛、嘴角、脸部肌肉、呼吸状态；
6. 潜动作：下意识小动作，例如摩挲戒指、按住衣袖、避开视线、手指收紧；
7. 环境对抗：角色如何受到风、雨、光、尘土、温度、空间压迫等影响；
8. 物理惯性：衣摆、头发、袖口、饰品、道具如何随动作自然运动；
9. 光影质感：局部光源、阴影层次、冷暖对比、皮肤质感，避免塑料感；
10. 声音语气：年龄感、声线质感、语速、情绪底色、发声习惯；
11. 视频用途：适合作为主角、反派、配角、封面人物、分镜关键帧、首帧图还是人物参考图。
...
【声音语气】
用“年龄感 + 声线质感 + 语速 + 情绪底色 + 发声习惯”的方式描述。

【角色设定提示词】
输出一段中文角色设定提示词。
如果用户没有指定画风，这段提示词必须明确包含“真人写实风格、电影感、真实光影、真实材质”。
如果用户指定了画风，这段提示词必须改用用户指定画风。
如果系统后续提供【本次提示词区块强制标题】，
必须使用那个标题，不再额外输出【中文绘图提示词】。

【可选英文关键词】
默认省略。只有用户明确要求英文/中英双语，
或需要给绘图工具补充风格词时才输出。
只给英文关键词，不整段翻译中文提示词。
...
【负面提示词】
避免：塑料皮肤、空洞眼神、僵硬表情、对称摆拍、手指畸形、衣服无物理惯性、过度磨皮、廉价大平光、文字水印、画面杂乱。
...
请按后端场景画面规则输出，聚焦空间、光影、氛围、构图和影像风格。`,
                instruction: `请你作为 AI 视频场景提示词专家，根据用户原始内容生成适合 AI 真人视频/写实视频制作的场景提示词。

你需要重点描述：
1. 场景类型：室内、室外、古风、现代、玄幻、赛博、校园、宫殿、街道等；
2. 时间与天气：清晨、黄昏、雨夜、雪天、月光、逆光等；
3. 空间结构：前景、中景、背景、主体位置、纵深关系；
4. 光影氛围：柔光、冷光、暖光、强对比、电影感、体积光；
5. 影像风格：写实、电影级、纪录片感、广告片感、古风、赛博朋克、复古胶片等；
6. 剧情作用：这个场景在故事里承担什么情绪或叙事功能。
...
只输出一份中文场景提示词。
如果系统后续提供【本次提示词区块强制标题】，
必须使用那个标题，不再额外输出【中文提示词】。

【可选英文关键词】
默认省略。只有用户明确要求英文/中英双语，
或需要补充风格关键词时才输出，不整段翻译。
【负面提示词】
...
请按后端表情语气规则输出，只聚焦脸部表情、眼神和声音/台词语气。`,
                instruction: `请你作为 AI 视频人物表情提示词专家，把用户输入的情绪转化成可用于 AI 真人视频/写实视频生产的人物表情、眼神、微动作和台词语气提示词。

不要只写“开心、愤怒、悲伤、震惊”这类抽象情绪。必须拆成：
1. 眼神：视线方向、瞳孔状态、眼眶、眼压、眨眼；
2. 眉毛：上挑、紧锁、下压、微皱；
3. 嘴部：嘴角、唇线、张口程度、咬唇；
4. 面部肌肉：脸颊、下颌、鼻翼、额头；
5. 身体反应：呼吸、肩膀、手部、小动作；
...
只输出一份表情/语气提示词。
如果系统后续提供【本次提示词区块强制标题】，
必须使用那个标题，不再额外输出【表情提示词】。
不要输出【动作提示词】或【视频提示词】。

【声音/台词提示词】
请用“年龄感 + 声线质感 + 语速 + 情绪底色 + 发声习惯”的方式描述。
...
请按后端分镜规则输出连续镜头，包含镜号、景别、镜头角度/运动和画面提示词。`,
                instruction: `请你作为 AI 视频分镜导演，把用户输入的剧情拆解成适合 AI 真人视频/写实视频制作的连续分镜。

每个分镜必须包含：
1. 镜号；
2. 景别：远景、全景、中景、近景、特写、大特写；
3. 镜头角度：平视、俯视、仰视、侧拍、背影、过肩镜头；
4. 镜头运动：推镜、拉镜、摇镜、跟拍、固定镜头；
5. 画面内容：人物、动作、场景、道具；
6. 人物表情；
7. 台词或旁白；
8. 情绪节奏；
...
| 镜号 | 景别 | 镜头角度/运动 | 画面内容 | 表情动作 | 台词/旁白 | 情绪作用 | AI画面提示词 |
|---|---|---|---|---|---|---|---|
...
请按后端动作规则输出，聚焦起始姿态、动作过程、力量方向、节奏和镜头建议。`,
                instruction: `请你作为 AI 视频动作设计师，把用户输入的动作拆解成适合 AI 真人视频生成、图生视频或分镜设计的动作提示词。
...
请按后端剧情规则输出剧情核心、人物关系、冲突推进、情绪弧线和结尾钩子；不要默认拆分镜。`,
                instruction: `请你作为 AI 视频剧情策划，根据用户输入生成适合 AI 真人视频/写实视频创作的剧情结构、冲突推进和创作提示词。

你需要重点分析：
1. 主角目标；
2. 阻碍力量；
3. 冲突类型：情感冲突、身份冲突、利益冲突、秘密揭露、误会反转；
4. 起承转合；
5. 爽点、虐点、悬念点；
6. 角色关系变化；
7. 适合延展成哪些关键场景，但不要拆成分镜表格；
8. 后续剧情钩子。
...
【关键情节点】
列出 3-5 个剧情节点，只写故事推进，不写镜号、景别或镜头运动。
...
{{input}}`,
    ...
    async loadPromptTemplatesFromServer() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/prompt-templates`, {
                credentials: "include",
            });
            ...
            Object.entries(templates).forEach(([templateKey, template]) => {
                if (!this.promptTemplates[templateKey] || !template.instruction) return;
                this.promptTemplates[templateKey] = {
                    ...this.promptTemplates[templateKey],
            ...
            console.warn("Load prompt templates failed, using local defaults", error);
        }
    }
    ...
    buildModelQuestion(question) {
        ...
        const instruction = template.runtimeInstruction || template.instruction;
        return instruction.replace("{{input}}", question.trim());
    }
